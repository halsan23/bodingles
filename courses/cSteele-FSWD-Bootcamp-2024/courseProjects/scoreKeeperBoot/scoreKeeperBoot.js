...
function updateScores (player, opponent) {
   if (!gameOver) {                       // is the game over?
      ...
      if (player.score === winScore) {    // if player score = winning score,
         ...
         player.display.classList.add('text-success');     // change text color for win
         opponent.display.classList.add('text-danger');   // change text color for lose
         player.button.disabled = true;     // disable button on a win
         opponent.button.disabled = true;     // disable button on a win
...
function reset() {
   ...
   for (let p of [playerOne, playerTwo]) {
      ...
      winLose.innerText = "Use the buttons below to play.";
   }
}